 ...
 * INTERNAL API
 */
@InternalApi private[akka] final class FlattenMergeWithContext[T, Ctx, M](
                                                                           val breadth: Int,
                                                                           val strategy: ContextMapStrategy.Iterate[T, Ctx, T])
  extends GraphStage[FlowShape[Graph[SourceShape[(T, Ctx)], M], (T, Ctx)]] {
  private val in = Inlet[Graph[SourceShape[T], M]]("FlattenMergeWithContext.in")
  private val out = Outlet[T]("FlattenMergeWithContext.out")
  ...
  override def createLogic(enclosingAttributes: Attributes) = new GraphStageLogic(shape) {
    ...
    def addSource(source: Graph[SourceShape[T], M]): Unit = {
      ...
      TraversalBuilder.getSingleSource(source.asInstanceOf[Graph[SourceShape[AnyRef], M]]) match {
        ...
        case _ =>
          val sinkIn = new SubSinkInlet[T]("FlattenMergeWithContext")
          sinkIn.setHandler(new InHandler {
            override def onPush(): Unit = {
              if (isAvailable(out)) {
                push(out, sinkIn.grab())
